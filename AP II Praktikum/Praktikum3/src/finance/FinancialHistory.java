...
// IMap und LinearMap brauchen nicht importiert werden, da diese als public
// deklariert wurden. Sichtbar fuer alle Klassen die sich im selben Paket befinden
...
public final class FinancialHistory {
     ...
     */
    private double cashOnHand;	// wurde irrtuemlich auf 100 gesetzt
    /**
     ...
     */
    public static FinancialHistory initialBalance(double amount) {			//WIESO??
        require(amount >= 0, "Amount " + amount + " is not positive");
        FinancialHistory f = new FinancialHistory();
        f.cashOnHand = amount;			// 'cashOnHand' koennte ohne 'f' nicht aufgerufen werden, da methode static ist
        return f;
    }
    ...
    public void receiveFrom(String source, double amount) {
    	...
    	require(amount >= 0, "Amount " + amount + " is not positive");
    	incomes.put(source, this.totalReceivedFrom(source) + amount);  // addiert wert auf vorhandenen Wert in der Map
    	cashOnHand += amount;
    }
    ...
    private static double totalOf(Collection<Double> values) {
        // TODO: Summe alle Zahlen in values berechnen.
        // Hinweis: foreach-Schleife benutzen
    	Object[] myCollection = values.toArray();	// speichert Werte von 'values' in unseren 'myColection' Objekt-Array
    	double sum = 0.0;
